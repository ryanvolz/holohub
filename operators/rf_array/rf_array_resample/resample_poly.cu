...
void ResamplePoly::initialize() {
  HOLOSCAN_LOG_INFO("ResamplePoly::initialize()");
  ...
  matx::make_tensor(filter, {filter_len});
  cudaMemcpy(
  ...
  matx::make_tensor(padded_data, {padded_len, num_subchannels.get()});
  ...
  matx::make_tensor(padded_out_data, {padded_out_len, num_subchannels.get()});

  HOLOSCAN_LOG_INFO("ResamplePoly: pad_size {}, out_pad_size {}, out_chunk_size {}",
                    pad_size,
                    out_pad_size,
                    out_chunk_size);

  HOLOSCAN_LOG_INFO("ResamplePoly::initialize() done");
}
...
void ResamplePoly::compute(InputContext& op_input, OutputContext& op_output, ExecutionContext&) {
  ...
  auto end_of_prior_prior = padded_data_flipped.Slice({0, chunk_size.get()},
                                                      {matx::matxEnd, pad_size + chunk_size.get()});
