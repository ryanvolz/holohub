...
void TypeConversionComplexIntToFloat::setup(OperatorSpec& spec) {
  spec.input<std::shared_ptr<std::vector<RFArray<sample_t>>>>("rf_in");
...
void TypeConversionComplexIntToFloat::compute(InputContext& op_input, OutputContext& op_output,
  ...
  auto in_msg_ptr =
      op_input.receive<std::shared_ptr<std::vector<RFArray<sample_t>>>>("rf_in").value();
  ...
  for (auto in : (*in_msg_ptr)) {
    ...
    out_msg_ptr->emplace_back(complex_data, in.metadata);
  }
