...
struct BufferTracking {
  ...
  auto completed_at_pos(size_t buf_idx, matx::tensor_t<sample_t, 3>& rf_data, cudaStream_t stream) {
    // Do accounting first, before issuing async buffer copy or tracking reset because there
    // might be other streams out there copying the tracking info from device to host memory
    // and we don't want to modify these host values before we use them by resetting the device
    // tracking values.
    unsigned long long this_pos = pos;
    ...
    for (size_t i = 1; i <= buffer_size; i++) {
      ...
      // We will only do this once for a given buffer because we are only allowing buffers with
      // counter_h[clr_buf_idx] between this_pos and counter_h[buf_idx] (inclusive) and pos
      // for next time has already been updated to be (counter_h[buf_idx] + 1)
      if (counter_h[clr_buf_idx] >= this_pos && counter_h[clr_buf_idx] <= counter_h[buf_idx] &&
